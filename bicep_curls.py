...
def reset():
    ...
    global counter
    global stage
    global extended_frames
    global curled_frames
...
# ============================================================
# BICEP CURL ALGORITHM
# ============================================================
#
# Tracks the elbow angle (shoulder - elbow - wrist). The rep is
# counted the moment the arm reaches the curled (contracted)
# position, the same "count on contraction" pattern situps.py
# uses for the crunch.
#
...
def detect_curl(landmarks):
    ...
    if smoothed_angle > EXTENDED_ANGLE:

        extended_frames += 1
        curled_frames = 0

        if extended_frames >= POSITION_FRAMES_REQUIRED:

            stage = "EXTENDED"

    # --------------------------------------------------------
    # CURLED (COUNTS THE REP)
    # --------------------------------------------------------

    elif smoothed_angle < CURLED_ANGLE:

        curled_frames += 1
        extended_frames = 0

        if curled_frames >= POSITION_FRAMES_REQUIRED:
